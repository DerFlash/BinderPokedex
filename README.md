...
*The left page uses the exact promoted Base Set poster cuts. On the right, the
first nine BinderPokedex positions are shown with matching real cards already
collected in positions #002, #004, #006, and #008.*

### Actual v9.0 PDF output

<p align="center">
  <img src="docs/images/binder-pokedex-primal-poster-page.png" width="430" alt="Actual localized Primal 3x3 poster page from BinderPokedex">
</p>

This real localized Primal start page is generated by BinderPokedex and ready
to print as nine physical cards.

### Coming in v9.0: Poster Artwork for Every Binder

- **41 reviewed poster openings** cover every current set, Pokédex generation,
  and EX subsection.
- Each continuous scene is composed around the physical **3×3 card cuts**.
- Localized logos and facts are added reproducibly for every supported PDF
  language.
- Print the default cuttable page or keep the same artwork as one continuous
  full-page print.
- The canonical cover remains available when no poster is enabled and through
  the explicit `--skip-poster` fallback.
...
- **Reviewed poster start pages** for all 41 current set and section targets,
  with localized overlays and a reliable cover fallback
