...
| `PA-030` | Custom layouts can be generated without changing release assets | Done | The custom-layout workspace command clones manifests and deterministic source inputs below ignored `tmp/`; an explicit hybrid option copies unselected existing promotions into that workspace, while `BINDER_POKEDEX_POSTER_ASSETS` routes generation, promotion, validation, slicing, and PDF discovery without writing tracked release assets |
