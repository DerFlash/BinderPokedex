...
| DreamO v1.1 multi-reference control | Yes, by the isolated graph contract | Three separate post-BEN2 references are intact, but preflight A omits all subjects | Must pass the unchanged physical-card gate | Must be resolved in the common final pass | MPS graph proven; one compact prompt-binding preflight remains before any visual candidate |
