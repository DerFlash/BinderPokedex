...
| `PA-023` | A poster carries the semantic information of its preceding cover | Done | Set posters show localized set name, card count, release date, title/logo, and project identity; aggregate posters show section title, subtitle/region, card count, description/range, collection title, and project identity without repeating an identical upper section title inside the information panel; inline logo tokens use their tracked artwork instead of flattened text; the cover-only cutting hint/build timestamp remains operational metadata |
