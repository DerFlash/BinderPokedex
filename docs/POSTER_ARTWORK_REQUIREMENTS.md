...
| Qwen spatial-subject multi-reference | Planned isolated preflight: one empty target, sampler, and decode | Each source will occur exactly once across three spatial references; visual identity remains unproven | Each reference places one subject inside its final physical bottom card | Common final pass can invent the landscape around all three subjects; unproven | Selected bounded reuse of the installed Qwen stack; old duplicate-reference topology remains rejected |
...
| `PA-017` | Natural grounding may be generated jointly without losing identity or card safety | Stronger identity/spatial-control successor still open | Production remains on `identity_lock`. Generic regional SDXL and DreamO are closed under their recorded hard gates; the available MS-Diffusion and LAMIC integrations are not KISS-/MPS-safe on the 16-GB target. Before any new model download, one bounded Qwen preflight removes the old duplicated-subject topology and supplies three independent poster-shaped references, each containing one source exactly once at its final card position, to one empty-target sampler/decode graph |
