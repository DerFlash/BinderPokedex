...
| Spatial+Identity true one-shot | Yes | Unscaled identity references are explicit anatomy authority, but `00017` still simplifies Litten's paw and Popplio's face details | `00017` passes all three card crops | Strong in `00017` | Implemented and rejected under the hard identity gate |
...
| `PA-017` | Natural grounding may be generated jointly without losing identity or card safety | Identity-control successor evaluation in progress | Production remains on `identity_lock`. The selected successor experiment keeps one common final model pass but replaces generic FLUX.2 reference latents with explicit per-subject identity and spatial controls. Generation VII compares generic SDXL identity control, the identical graph with a Pokémon domain LoRA, and a compatible FLUX.2 4B RefControl spike. No candidate is promoted unless it passes every existing hard gate without post-decode character compositing or restoration |
