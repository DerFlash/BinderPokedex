...
| `PA-017` | Natural grounding may be generated jointly without losing identity or card safety | Stronger identity/spatial-control successor still open | Production remains on `identity_lock`. Generic regional SDXL is closed after three hard-gate failures; the available MS-Diffusion port is rejected because making its hidden CUDA/xFormers Diffusers sampler MPS-safe and auditable would require a substantial fork. DreamO v1.1 completes one empty-target sampler/decode graph on MPS, but compact-prompt preflight B binds only two redesigned, oversized subjects and omits the third. It is closed before 1 MP without adding masks, per-subject passes, or restoration. The next architecture must expose both multi-reference identity and explicit spatial assignment before model download |
