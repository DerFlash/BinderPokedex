...
| FLUX.2 Spatial+Identity v5 true one-shot | Yes | `00018` and `00019` retain the recognizable designs within the explicitly accepted print-detail tolerance; gross anatomy, marking, silhouette, and form changes remain forbidden | Both reuse the exact canonical Option-1 placement profile and pass all three physical card crops with preferred fill | Strong grounding and shadows; `00019` ignores the explicit front/rear crossing stress rule | Ranked second through `00018`; one final compact-prompt test is allowed because the current 1,350-token prompt exceeds the intended 512-token FLUX.2 budget |
...
| `PA-017` | Natural grounding may be generated jointly without losing identity or card safety | Final FLUX.2 v5 depth test open | `identity_lock` remains the first-ranked production baseline. v5 `00018` passes card fit and `00019` proves that a late rule inside the 1,350-token prompt does not cause intersections. One final same-seed candidate may only compact/reorder that prompt to the intended 512-token budget and test one connected front/rear crossing; prompt tuning stops afterward |
