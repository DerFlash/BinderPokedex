...
class FeaturedPokemonRenderer:
    ...
    def draw_iconic_pokemon(canvas_obj, iconic_ids: List[int], pokemon_list: list, 
        ...
        for idx, poke_id in enumerate(clean_iconic_ids[:3]):
            ...
            # Try to get image source from Pokemon data first, then fall back to cache
            image_source = None
            if pokemon:
            ...
            try:
                ...
                if image_source:
                    # Pokemon exists in list with image source
                    ...
                    # Pokemon not in list - try to load directly from cache
                    # This handles featured Pokemon that don't exist in variant Pokemon lists
                    logger.debug(f"Featured Pokemon #{poke_id} not in pokemon_list, loading from cache")
                    image_to_render = image_cache.get_image(poke_id, 
                                                            url=None,  # No fallback URL
                                                            timeout=3,
                                                            size='featured')
